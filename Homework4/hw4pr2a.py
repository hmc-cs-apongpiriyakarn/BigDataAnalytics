...
June 5, 2019
MATH189R Big Data Analytics Homework 4 Problem 2a
"""
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		# Calculate the negative log likelihood and append value in nll_list
...
def newton_step(X, y, W, reg=0.0):
	...
	# Calculare Hessian matrix of logistic regression
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		# Calculate negative log likelyhood and append value to nll_list
		nll = NLL(X, y, W, reg = reg)
		if np.isnan(nll):
			break
		nll_list.append(nll)

		# Calculate gradient for W using newton step and update W
...
def get_description(X, y, W):
	...
	total_r = 0
	for i in range(m):
		actual, pred = y.item(i), y_pred.item(i)
		if actual == pred:
			count_a += 1
		if actual == 1:
			total_r += 1
	...
	# Precision = probability of true label being 1 given that the predicted label is 1
	precision = 1. * count_p / total_p
	# Recall = probablity of predicted label being 1 given that the true label is 1
...
if __name__ == '__main__':
	...
	# for logistic regression, we only want images with label 0 or 1.
	...
	# Fill in the code in grad_logreg, NLL and grad_descent for this step
	...
	# Fill in the code in newton_step and newton_method for this step
	...
	# Fill in the code in get_description and plot_description for this Step
